...
def parse_args():
    ...
    # model
    parser.add_argument('--arch', '-a', metavar='ARCH', default='LGMANet')
...
def train(config, train_loader, model, criterion, optimizer):
    ...
    for input, target, _ in train_loader:
        ...
        if config['deep_supervision']:
            ...
            loss /= len(outputs)
            iou, dice, pre, recall  = iou_score(outputs[-1], target)
        else:
            output = model(input).to(device)
            loss = criterion(output, target).to(device)
            iou, dice, pre, recall = iou_score(output, target)
...
def main():
    ...
    # create model
    model = archs1.__dict__[config['arch']](config['num_classes'],
    ...
    for epoch in range(config['epochs']):
        ...
        print('loss %.4f - iou %.4f - val_loss %.4f - val_iou %.4f - val_pre %.4f - val_recall %.4f'
              % (train_log['loss'], train_log['iou'], val_log['loss'], val_log['iou'], val_log['pre'], val_log['recall']))
